...
// 0x5a8d0F6596fd4C81608BB9E5905d7A1aF82294b3
        ...
        if (error) {
            ...
            for (i = 0; i < number_of_clients; i++) {
                myContract.valid_client_list.call(i, {
                    ...
                    alert("1111111");
                    if (client_addr.equals(result)){
                        alert("The address has been registered! Please choose a new one!");
                        break;
                    }
                })
...
function insert_age() {
    ...
    // }
    // var numArr = document.getElementsByName("address1");
    ...
    // newNumArr = numArr.substr(12);
    // alert(numArr);
    myContract.insert_age(age, numArr, {
        from: user.account,
        gas: 4200000
    }, function (error, result) {
        if (error) {
...
function insert_degree() {
    ...
    myContract.insert_degree(degree, description, numArr, {
        from: user.account,
        gas: 4200000
    }, function (error, result) {
        if (error) {
...
function insert_license() {
    ...
    myContract.insert_license(license, description2, date, numArr, {
        from: user.account,
        gas: 4200000
    }, function (error, result) {
        if (error) {
...
function check_organization() {
    myContract.valid_organizations.call(account, {
        from: user.account,
        gas: 4200000
    }, function (error, result) {
        // alert(user.account);
...
function check_validators() {
    myContract.valid_validators.call(account, {
        ...
        // alert(result);
        if (result) {
            window.open("validator/index.html");
        } else {
...
window.onload = function(){
    ...
    myContract.total_valid_validators.call(function (error, result) {
        if (error) {
            ...
            var number_of_vals = result;

            for (var i = 0; i < number_of_vals; i++) {
                myContract.valid_validators_list.call(i, {
                    from: user.account,
                    gas: 4200000
                }, function (error, result) {
                    // alert(result);
                    document.getElementById("validlists").innerHTML += "<li>" + 'id: Account'+result.substring(2,5)+'-'+result + "</li>";
                })
        ...
        if (error) {
            ...
            for (var i = 0; i < number_of_vals; i++) {
                myContract.valid_validators_list.call(i, {
                    ...
                    // alert(result);
                    // document.getElementById("validlists").innerHTML += "<li>" + 'id: Account'+result.substring(2,5)+'-'+result + "</li>";
                    document.getElementById("validvalidatorlist").innerHTML += "<option>" +result + "</option>";
                })
